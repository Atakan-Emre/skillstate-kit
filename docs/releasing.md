...
2. Update CHANGELOG and regenerate `uv.lock`. Run `scripts/build_integrations.py` and `scripts/build_readme.py`; commit refreshed distribution assets.
...
See [validation](validation.md) for published release evidence. Registry presence and workflow results, not a prepared document, establish that a new publication actually happened.
...
The optional `testpypi` input adds a rehearsal before production using the same artifacts and hash/installation verification. It requires a separate TestPyPI account and publisher. Production publication follows CI validation; TestPyPI is optional. A requested rehearsal that fails blocks production.
...
After publication, verify the version-specific PyPI JSON endpoint (for example, `https://pypi.org/pypi/skillstate-kit/VERSION/json`), the exact uploaded hashes, and clean installation using `python -m pip install skillstate-kit==VERSION`, substituting the version being released. Tag the tested release commit and update the release notes with the publishing run URL. Never reuse a published version for changed distribution bytes.
