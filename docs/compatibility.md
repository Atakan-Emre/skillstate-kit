...
| Host | Skill output | Optional project MCP config | Integration surface |
|---|---|---|---|
...
Use the [canonical installation guide](integrations.md) for explicit host selection, MCP/CLI-only setup, upgrading and removal. Keep machine-specific configuration local. Reload the host after setup and respect its project trust settings.
...
No hook installation is shipped. Native tool interception and host transcript replacement are not promised. The generated skill asks the host to record state/operations, and compliant calls are enforced by the library. If the host skips the protocol, the library cannot reconstruct invisible effects.
