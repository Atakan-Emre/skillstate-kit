...
Live acceptance evidence: Codex used eight real MCP calls to record a synthetic invoice review and hand it to Claude Desktop. After an initial permission/timeout interruption, Desktop resumed the same run, saved a second-review artifact and completed at revision 4. A separate process verified the stored state, both artifacts and the event history. The tested Antigravity desktop session reported MCP tools unavailable, and its separate IDE required login. This representative Codex-to-Claude Desktop Chat success does not establish universal desktop compatibility.
